...
for (i in 1:it) {
  ...
  Task_ML_A1_0<- TaskRegrST$new(id =  "ML_A1", data_t0[,-c(which(colnames(FoIobs_A1)=="Code"),which(colnames(FoIobs_A1)=="MedFoI"))], target = "logMedFoI"
  ...
  data_t<- data_t0[,colnames(data_t0)%in% c(Keep, "logMedFoI", "lat", "lon", "year")]
  Task_ML_A1<- TaskRegrST$new(id =  "ML_A1_f",data_t, target = "logMedFoI"
                              , extra_args = list(coords_as_features = T, crs = NA, coordinate_names = c( "lat", "lon", "year")))
  ...
  dat0<- as.data.frame(cbind(FoIobs_A1$MedFoI, exp(pred_obs_t$response), FoIobs_A1$IndiZone, FoIobs_A1$UrbZone,  FoIobs_A1$RurZone, FoIobs_A1$Code, FoIobs_A1$lon, FoIobs_A1$lat, FoIobs_A1$year))
  ...
  #v<-variogram(sp_cvDat$resi~1, sp_cvDat)
  #plot(v)
  #fit.variogram(v, vgm(1,"Sph", 1))
  
  #bubble( sp_cvDat,zcol="resi")
 ...
 # plot(dat$year,dat$resi_norm)
 # qqnorm(dat$resi_norm)
 # qqline(dat$resi_norm)
 # acf(dat$resi_norm, lag=50)
 # plot(dat$resi_norm~ dat$year)
  ...
  for (y in 1:length(years)){
    ...
    pred_new_urb[,y,i]<- exp(pred_new_urb_t$response)
    ...
    pred_new_rur[,y,i]<- exp(pred_new_rur_t$response)
    ...
    pred_new_ind[,y,i]<- exp(pred_new_ind_t$response)
  }
}
