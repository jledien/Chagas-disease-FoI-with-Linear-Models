...
for (i in 1:it) {
  ...
  for (b in 1:boot){
    ...
    # task
    Task_ML_A3_0<- TaskRegrST$new(id =  "ML_A3",Boot_it_set[,-c(which(colnames(Boot_it_set)=="Code"),which(colnames(Boot_it_set)=="rn") , which(colnames(Boot_it_set)=="FoI") ,
                                                                which(colnames(Boot_it_set)=="row_code"))] , target = "logFoI"
                                  , extra_args = list(coords_as_features = T, crs = NA, coordinate_names = c( "lat", "lon", "year")))
    
    
    # training
    lrn_SE$train(Task_ML_A3_0)
    var_1 <-lrn_SE$importance()
    Keep<-  names(var_1[1:10])# Keep the 10 most important variable for the model
    
    data_t<- Boot_it_set[,colnames(Boot_it_set)%in% c(Keep, "logFoI", "lat", "lon", "year")]
    Task_ML_A3<- TaskRegrST$new(id =  "ML_A3",data_t, target = "logFoI"
                                , extra_args = list(coords_as_features = T, crs = NA, coordinate_names = c( "lat", "lon", "year")))
    # variable importance
    lrn_SE$train(Task_ML_A3)
    var_2 <-lrn_SE$importance()
    for (n in 1:length(features)){
      var_imp[n,((i-1)*boot+b)]<- ifelse(features[n] %in% names(var_2), var_2[which(names(var_2)==features[n])],0)
    }
    ...
    d_pred<-  as.data.frame(cbind(as.character(FoIobs_A1$Code), exp(pred_obs_t$response )))
   ...
   for (y in 1:length(years)){
      ...
      pred_new_urb[,y,((i-1)*boot+b)]<- exp(pred_new_urb_t$response)
      ...
      pred_new_rur[,y,((i-1)*boot+b)]<- exp(pred_new_rur_t$response)
      ...
      pred_new_ind[,y,((i-1)*boot+b)]<- exp(pred_new_ind_t$response)
    }
  }
}
