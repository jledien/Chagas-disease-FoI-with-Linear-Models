...
for (i in 1:it) {
  ...
  for (b in 1:boot){
    ...
    data_t<- Boot_it_set[,colnames(Boot_it_set)%in% c(Keep$feature, "logFoI", "lat", "lon", "year")]
    ...
    for (n in 1:length(features)){
      var_imp[n,((i-1)*boot+b)]<- ifelse(features[n] %in% var_2$feature, var_2$score[which(var_2$feature==features[n])],0)
    }
