...
ADM2_Predictors_AllADM2 <- function(year, tSur, zone){
  
  Predictors <- readRDS( "Comparison_with_LinearModels/predictors_ADM2")
  ...
  Predictors$RurZone <- ifelse(Predictors$zone=="rural", 1,0)
  ...
  Predictors$TempPopDensity [Predictors$GID_2_GDAM=="COL.26.1_1"] <-mean(Predictors$TempPopDensity[Predictors$DP==88], na.rm=T) 
  
  Predictors$PolarClimatePresence<- ifelse(Predictors$PolarFreq>0,1,0)
  
  data<- Predictors[,c(1:18,29:35,73:87)]
...
ADM2_Predictors <- function(year, tSur, zone, ADM2_GDAM_Code){
  ...
  Predictors$TempPopDensity [Predictors$GID_2_GDAM=="COL.32.4_1"] <-mean(Predictors$TempPopDensity[Predictors$NAME_1_GDAM=="Vichada"], na.rm=T) 
  Predictors$TempPopDensity [Predictors$GID_2_GDAM=="COL.32.5_1"] <-mean(Predictors$TempPopDensity[Predictors$NAME_1_GDAM=="Vichada"], na.rm=T) 
  Predictors$TempPopDensity [Predictors$GID_2_GDAM=="COL.26.1_1"] <-mean(Predictors$TempPopDensity[Predictors$NAME_1_GDAM=="San Andrés y Providencia"], na.rm=T) 
  
  
  data<- Predictors[which(Predictors$GID_2_GDAM==ADM2_GDAM_Code),c(1:18,29:35,73:85)]
  
  return(data)
}
## the otput is the predictiors for one municipalities so can be use to build predictors in new subset of ADM2