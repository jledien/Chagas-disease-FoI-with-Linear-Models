...
for (i in 1:it) {
  ...
  for (b in 1:boot){
    ...
    data_t<- Boot_it_set[,colnames(Boot_it_set)%in% c(Keep, "FoI", "lat", "lon", "year")]
...
rownames(performance_table)<- c("R2","resampled_R2","R2_CV","R2_urb","R2_rur","R2_ind", "Ov","Ov_urb","Ov_rur","Ov_ind", "Ind","Ind_urb","Ind_rur","Ind_ind" , "Moran_I_stat", "Moran_I_p_value" , "DW_test_stat")
