...
for (i in 1:it) {
  ...
  filter1$calculate(Task_ML_A1_0)
  var_1 <-as.data.table(filter1)
  Keep<-  data.frame(var_1[1:10,1])# Keep the 10 most important variable for the model
  
  
  data_t<- data_t0[,colnames(data_t0)%in% c(Keep$feature, "logMedFoI", "lat", "lon", "year")]
  Task_ML_A1<- TaskRegrST$new(id =  "ML_A1",data_t, target = "logMedFoI"
                              , extra_args = list(coords_as_features = T, crs = NA, coordinate_names = c( "lat", "lon", "year")))
  
  # variable importance
  lrn_SE$train(Task_ML_A1)
  filter2<- flt("importance", learner=lrn_SE)
  filter2$calculate(Task_ML_A1)
