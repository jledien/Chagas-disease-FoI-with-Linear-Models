...
titles <- c('Median','up95','CV','up50','up50 scaled','mCV')
...
for (zone in 1:length(zone_unique)){
  ...
  for (year in Years){
    ...
    New_pred<- data.frame(GID_2_GDAM = pred_mat$GID_2_GDAM[1:1065],
                          ...
                          CV_MAD = Y_pred_colombia_sum_MA[[as.character(year)]][[zone]][,8])
    ...
    rgdal::writeOGR(temp2, paste0("Rdata/maps_CV_MAD/",approach), # maps: name folder 1 per approach
    ...
    New_pred$CV_MAD[which(New_pred$CV_MAD > 10)] <- NA
    ...
    q[[which(Years %in% year)]] <- ggplot(data = temp, aes(x=long, y=lat,
                                                           group = group, fill = CV_MAD))+
      geom_polygon()  + xlab('') + ylab('') +
      # geom_path(color = "white") +
      scale_fill_gradientn('',colours = terrain.colors(7), 
                           breaks = breaks_map2, limits = c(0,10)) +
      coord_equal() + ggtitle(paste0(titles[6],' - ',year)) +
      theme(axis.text = element_blank(), 
            legend.text=element_text(size=6),
            legend.key.height = unit(.3,'cm'),
            legend.key.width = unit(.2,'cm'))
