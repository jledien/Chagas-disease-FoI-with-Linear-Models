approach3_inference_bootstrap <- function(FoI_ADM2, obs_dist, models, 
                                          predictorlist, n_boot, p_infer){
  
  Max_obs_foi <- apply(obs_dist,1,max)
  
  n_sample_foi <- ncol(obs_dist)
  log_Obs_dist_foi <- log(obs_dist)
  ...
  for(i in 1:length(predictorlist)){
    ...
    for (k in 1:n_boot){
      ...
      #randomly choose from posterior distribution
      y_full_boot <- log_Obs_dist_foi[,sample(x = n_sample_foi,size = 1)]  # FoI_ADM2$logFoIMed[f_incl$f_infer[,k]]
      y <- y_full_boot[f_incl$f_infer[,k]]
